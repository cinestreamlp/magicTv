 ...
 */
public class France2Channel extends Channel {
    @Override
    public void getFolders(Callback<List<Folder>> callback) {
        new PluzzFolderLoader("france2").onSuccess(callback).execute();
    }

    @Override
    public void getTvProgram(Callback<TvProgram> callback) {
        new PluzzTvProgramLoader("france2", "France_2").onSuccess(callback).execute();
    }
}
